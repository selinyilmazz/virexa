        ...
        <div className="mx-auto max-w-[1820px]">
          <nav aria-label={t("article.breadcrumb.ariaLabel")} className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            <Link href="/open-source" className="transition-colors duration-200 hover:text-slate-700 dark:hover:text-slate-200">
              {t("openSource.breadcrumb.current")}
            </Link>
            <span aria-hidden="true">›</span>
            <span className="font-medium text-slate-950 dark:text-white">{t("openSource.topicsPage.breadcrumbCurrent")}</span>
          </nav>

          <div className="mt-4">
            <h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">{t("openSource.topicsPage.title")}</h1>
            <p className="mt-1.5 max-w-2xl text-base leading-relaxed text-slate-500 dark:text-slate-400">
              {t("openSource.topicsPage.subtitle")}
            </p>
          </div>

          <form action="/open-source/topics" method="GET" className="mt-6 max-w-md">
            <label htmlFor="topic-search" className="sr-only">
              {t("openSource.topicsPage.searchLabel")}
            </label>
            <div className="flex h-12 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 shadow-sm transition-colors focus-within:border-slate-300 dark:border-slate-800 dark:bg-slate-900">
              ...
              <input
                ...
                type="search"
                defaultValue={query}
                placeholder={t("openSource.topicsPage.searchPlaceholder")}
          ...
          <p className="mt-6 text-sm font-medium text-slate-500 dark:text-slate-400">
            {t(filtered.length === 1 ? "openSource.topicsPage.topicSingular" : "openSource.topicsPage.topicPlural", { count: filtered.length.toLocaleString("en-US") })}
            {query && (
              <>
                <span className="text-slate-400"> • </span>
                {t("openSource.topicsPage.matching", { query })}
              </>
            )}
          </p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {pageItems.map((topic) => (
              ...
              >
                ...
                <span className="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                  {t(topic.count === 1 ? "openSource.topicsPage.repoCountSingular" : "openSource.topicsPage.repoCountPlural", { count: topic.count })}
                </span>
              </Link>
            ))}
            ...
            {pageItems.length === 0 && (
              <div className="col-span-full rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center text-sm text-slate-500 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                {t("openSource.topicsPage.noTopicsMatch", { query })}
              </div>
            )}
          </div>
