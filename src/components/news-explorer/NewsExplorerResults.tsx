 ...
 */
export async function NewsExplorerResults({ items, highlightQuery }: NewsExplorerResultsProps) {
  if (items.length === 0) {
    ...
    return (
      <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 px-6 py-16 text-center">
        ...
        <h3 className="mt-6 text-xl font-bold tracking-tight text-slate-950">{t("explorer.noResults.title")}</h3>
        <p className="mt-2 max-w-md text-base leading-relaxed text-slate-500">{t("explorer.noResults.subtitle")}</p>
      </div>
    );
  }
