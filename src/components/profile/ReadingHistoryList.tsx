  ...
  if (status === "loading" || status === "idle") {
    return (
      <div className="flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-6 py-12 text-sm text-slate-500">
        {t("profile.readingHistory.loading")}
      </div>
    );
  }
  ...
  if (status === "error") {
    return (
      <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-red-200 bg-red-50/40 px-6 py-12 text-center">
        <p className="text-sm font-medium text-red-600">{error ?? t("profile.readingHistory.loadError")}</p>
        <button
          type="button"
  ...
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white px-6 py-12 text-center">
        ...
        <h3 className="mt-4 text-lg font-bold tracking-tight text-slate-950">{t("profile.readingHistory.emptyTitle")}</h3>
        <p className="mt-1.5 max-w-sm text-sm leading-relaxed text-slate-500">{t("profile.readingHistory.emptySubtitle")}</p>
      </div>
    );
  }
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500">
              ...
              <span>{t("profile.readingHistory.lastViewed", { date: formatPublishedDate(item.readAt) })}</span>
              {item.readingTime && (
                <>
                  <span aria-hidden="true">·</span>
          ...
          >
            {t("profile.readingHistory.continueReading")}
          </span>
        </article>
      ))}
    </div>
