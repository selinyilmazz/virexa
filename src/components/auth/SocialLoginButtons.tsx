...
type SocialLoginButtonsProps = {
  /** May resolve `{ ok: false }` (see `signInWithOAuthProvider`) when the flow fails to start - a successful call never resolves, since the browser navigates away first. */
  onGoogleClick: () => void | Promise<void>;
  disabled?: boolean;
};

/**
 * Real Google OAuth trigger (see `signInWithOAuthProvider` in
 * `src/lib/supabase/oauth.ts` for what `onGoogleClick` actually does).
 * Owns its own loading state independent of the parent form's
 * `isSubmitting` - clicking "Continue with Google" disables the button
 * and swaps in a spinner + "Signing in..." immediately, per the
 * production auth flow spec, rather than waiting on any other state.
 */
export function SocialLoginButtons({ onGoogleClick, disabled }: SocialLoginButtonsProps) {
  ...
  async function handleClick() {
    ...
    try {
      ...
      // Only reached if the flow failed to start (provider disabled,
      // network error) - a successful `signInWithOAuth` call navigates
      // the browser to Google before this line would ever run.
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        {isGoogleLoading ? t("auth.googleSigningIn") : t("auth.google")}
      </button>
    </div>
  );
}
