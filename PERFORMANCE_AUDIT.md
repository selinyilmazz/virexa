...
## 7. Traffic-spike defensive measures (lowest-risk tier) — implemented

Follow-up scenario: a sudden 126,000 requests/day to article/category pages. Rather than reaching for the rendering-architecture migration (§6, still not implemented - unverified build, out of scope for a rapid response), two lowest-risk measures were implemented, in this order:

1. **`robots.txt`** (`src/app/robots.ts`) - added a second rule disallowing the whole site for `AhrefsBot`, `SemrushBot`, `MJ12bot` (Majestic), and `DotBot` (Moz) - third-party SEO/backlink-analysis crawlers that scrape broadly for their own paid tooling, with no referral traffic or indexing benefit to Virexa in return. The default `*` rule (real search engines, AI/LLM crawlers, everyone else) is untouched. Advisory only, zero runtime cost, fully reversible (static file) - a compliant bot honors it for free; it does nothing against a bot that ignores `robots.txt`.

2. **Middleware throttling** (`src/middleware.ts`, `src/lib/bots/is-bot-request.ts`) - added `isSeoCrawlerUserAgent()`, the same narrow Ahrefs/Semrush/Majestic/Moz set (never search engines or AI crawlers), and a `throttleSeoCrawler()` check that runs *before* `updateSession()` and every other existing check in `middleware.ts`. Over 12 requests/60s from one IP in that UA set gets a `429` with `Retry-After`, at the cost of one in-memory `Map` lookup - no Supabase call, no page-render invocation. `/robots.txt` and `/sitemap.xml` are exempted so a crawler can still discover the disallow rule above. Reuses the existing `checkRateLimit`/`getClientIdentifier` primitives from `rate-limit.ts` (previously only wired to `POST /api/metrics`) rather than adding new infrastructure - same documented limitation carries over: in-memory, per server instance, so this dampens rather than hard-caps under heavy serverless fan-out.

Scope discipline, per explicit instruction: only these three files changed. `next.config.ts`, `layout.tsx`, `resolve-locale.server.ts`, `supabase/server.ts`, and everything under `src/i18n/` are untouched - verified via `git diff --stat` and `tsc --noEmit` (clean). No rendering architecture, Cache Components, auth, or localization behavior changed.

**What this does and doesn't do:** both measures act on *invocation count*, not CPU-per-invocation - a blocked/throttled request never reaches the render Function at all. They only address the narrow "known SEO-tool crawler" slice of traffic, by design (the explicit ask was non-search-engine SEO crawlers specifically). If most of a 126,000-request spike comes from sources outside that UA set - other scrapers, a broader bot swarm, or genuine traffic growth - these two measures won't touch it. That's still the open question from the prior turn: pulling real Vercel Observability data (UA breakdown, repeat-vs-unique path distribution) is what determines whether this tier is sufficient or whether the higher-risk tiers (§6/§7 of the migration plan, or a Vercel Firewall rule) become necessary.
